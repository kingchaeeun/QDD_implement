...
class QuoteDetector {
  private quotes: Map<string, string> = new Map();

  /**
   * 기사 본문 DOM 루트를 반환
   * - 사이드바/헤더 등 불필요한 영역은 제외하기 위해 사용
   */
  private getArticleRootElement(): HTMLElement | null {
   ...
   */
  private getHeadlineElements(): HTMLElement[] {
  ...
  detectQuotes(): Array<{ id: string; quote: string; position: number }> {
    ...
    const detectedQuotes: Array<{ id: string; quote: string; position: number }> = [];
    let quoteIndex = 0;
  ...
  getFullQuote(quoteId: string): string | undefined {
   ...
   */
  highlightQuotes(quotes: Array<{ id: string; quote: string; position: number }>): void {
    ...
    const quoteQueueByText = new Map<string, string[]>();
    const quoteNumberById = new Map<string, number>();
...
class ContentScriptManager {
  private detector: QuoteDetector = new QuoteDetector();

  /**
   * 페이지 오른쪽에 인라인 사이드패널(iframe)을 생성/표시
   * - Chrome sidePanel API 제약(사용자 제스처) 회피용
   */
  private openInlineSidePanel(): void {
  ...
  private detectAndHighlight(): void {
  ...
  private addQuoteClickListeners(quotes: Array<{ id: string; quote: string; position: number }>): void {
    const quoteMap = new Map(quotes.map((q) => [q.id, q]));
    const marks = document.querySelectorAll<HTMLElement>("mark.quote-highlight");
  ...
  private async onQuoteClick(quoteData: any): Promise<void> {
   ...
   */
  private runBackendForAllQuotes(quotes: Array<{ id: string; quote: string; position: number }>): void {
    if (!quotes.length) return;
